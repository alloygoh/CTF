...
##
#
# credits: https://github.com/just-max/picoctf2018
class Poracle
  private
  ...
  def _find_character_decrypt(character, block, previous, plaintext, character_set)
    ...
    if(@do_decrypt.call(failblock1) and @do_decrypt.call(failblock2))
      raise("Two different blocks with different padding succeeded; I'm not sure that your do_decrypt function is returning correctly?")
    end
  ...
  def encrypt(data)
    ...
    last_block = (0...@blocksize).map { 'A' }.join
    result = last_block
